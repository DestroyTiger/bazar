...
const cardIeams = document.querySelectorAll(".card-img-resize");
const spanMore = document.querySelectorAll(".spanmore");
...
cardIeams.forEach((item,index) => {
    cardIeams[index].addEventListener("mouseover", cardItemChangeOver);
    let i=index;

    function cardItemChangeOver() {
        spanMore[i].innerHTML = "کمتر";
        svgArrow[i].style.animation="svgarrow .4s forwards";
        cardIeams[i].classList.toggle("card-img-resize-active")
    }
    cardIeams[index].addEventListener("mouseout", cardItemChangeOut);

    function cardItemChangeOut() {
        spanMore[i].innerHTML = "بیشتر";
        svgArrow[i].style.animation="svgarrow .5s  reverse";
        svgArrow[i].style.animation="";
